module Make (L : Theory.Literals) = struct
  module Literal = struct
    let neg (x : L.t) : L.t =
      match x with L.Neg v -> L.Pos v | L.Pos v -> L.Neg v

    let symbol (x : L.t) : L.A.t = match x with Neg v -> v | Pos v -> v
    let value (x : L.t) : bool = match x with Neg _ -> false | Pos _ -> true

    let is_non_false (x : L.t) (f : L.A.t -> bool option) : bool =
      f (symbol x)
      |> CCOption.map (fun interpretation ->
             Bool.equal interpretation (value x))
  ...
  let rec rewrite_to_nnf_acc (f : uc_formula) (negate : bool) : nnf_formula =
    match (f, negate) with
    | And (x, y), true ->
        NOr (rewrite_to_nnf_acc x true, rewrite_to_nnf_acc y true)
    | And (x, y), false ->
        NAnd (rewrite_to_nnf_acc x false, rewrite_to_nnf_acc y false)
    | Or (x, y), true ->
  ...
  and rewrite_to_cnf_or (e1 : nnf_formula) (e2 : nnf_formula) : nnf_formula =
    ...
    match (re1, re2) with
    | NAnd (f, g), NAnd (a, b) ->
        NAnd (NAnd (NAnd (NOr (f, a), NOr (f, b)), NOr (g, a)), NOr (g, b))
