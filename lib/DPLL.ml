...
module SolverState(T: Theory.S) = struct 
  ...
  let assign_true_in_state (st: t) (l: assignment) = 
    ...
    CCList.iter (Clauses.assign_true) affected_clauses; 
      ...
      model = assigned; unassigned = unassigned; stack = rstack; occ_clause = st.occ_clause; theory_state= st.theory_state} 

  type reduction_result =
    | TheoryConflict of LitSet.t
    | Conflict
    | Result of t
  ...
  let perform_theory_update (st: T.state) (just_assigned: T.L.t): [`Implied of (LitSet.t* T.state) | `Conflict of LitSet.t] =
  ...
  let rec unit_prop_aux (st: t) (just_assigned: T.L.t) (to_assign: LitSet.t): reduction_result  =
    ...
    | `Implied (implied_lits, nst) ->
      ...
      | Some(st,assignees) ->
        if LitSet.is_empty assignees then Result st else
          ...
          let st  =  assign_true_in_state st {target=next_assign;kind=Consequence} in
  ...
  let undo_literal_assignment (st:t) (l: T.L.t) (to_undo_clauses: Clauses.t list): t = 
  ...
  let rec backtrack (st: t): (t * T.L.t)  option = 
    ...
    | (a,to_undo_clauses) :: rst_stack ->
        ...
        match a.kind with 
        | Dec tstate -> Some ({undid_value with theory_state=tstate}, a.target)
  ...
  let add_clause_to_state (st: t) (watch_list: LitSet.t) (clause: LitSet.t): t = 
    let dedup = LitSet.to_list clause in
    let cls =   ({Clauses.active=true; Clauses.cl=dedup; Clauses.watched=watch_list} |> ref) in
    let new_bindings = CCList.map (fun l -> (l,cls :: OccM.find l st.occ_clause)) dedup in 
    let new_occ = OccM.add_list st.occ_clause new_bindings in
    let new_watcher_bindings = CCList.map  (fun l -> (l,cls :: OccM.find l st.watch_lists)) (LitSet.to_list watch_list)  in
    let new_watchers = OccM.add_list st.watch_lists new_watcher_bindings in
   {stack=st.stack;theory_state=st.theory_state;formula=(cls :: st.formula); unassigned=st.unassigned;model=st.model;
    watch_lists=new_watchers;occ_clause=new_occ}
  ...
  let attempt_create_info_clause (st: t) (lset: LitSet.t): t option = 
    let still_has_conflict = LitSet.for_all (fun x -> F.Literal.is_definitely_true x (fun el -> M.ModelMap.get el st.model) ) lset in
    let flipped_to_things_we_want = LitSet.map (fun x -> F.Literal.neg x) lset in 
    let unassigned_2 = LitSet.filter (fun x -> 
     not (SymbSet.mem (F.Literal.symbol x) st.unassigned )) flipped_to_things_we_want |> LitSet.to_list |> CCList.take 2 in
    if CCList.length unassigned_2 = 2 then
     (*build new clause and add it*)
     Some (add_clause_to_state st (LitSet.of_list unassigned_2) flipped_to_things_we_want)
  ...
  let rec solve (st: t) (just_assigned: T.L.t option): [`Sat of t | `Unsat] =
    ...
    let next_state = if (CCOption.is_none just_assigned) then Some st else CCOption.flat_map (fun to_reduce -> 
      match unit_prop st to_reduce with
        | TheoryConflict conflicting_vars -> conflict_backtracing conflicting_vars st
    ...
    and solve from there *)
      | Some target_assignment ->
        ...
        solve (assign_true_in_state st {target=lit; kind=Dec st.theory_state}) (Some lit)) next_state)
  and   (* Invariant since if there was a cnflict we should have discovered it prior to our decision
    this  *)
    conflict_backtracing (conflicting_literals: LitSet.t) (st: t): t option =
      let rec backtrack_until_theory_consistent (st: t) =
        ...
        | None -> None
        | Some (nst,dec_lit) -> 
          if LitSet.is_empty conflicting_literals then 
            Some((nst, dec_lit)) 
          else 
      ...
      CCOption.flat_map (fun (nst, js) -> let flip_dec = F.Literal.neg js  in 
        sol_to_opt (solve (assign_true_in_state nst {target=flip_dec; kind=Consequence})  (Some flip_dec))) backtraced_state
  ...
  let solve_inject (f: F.cnf_formula): [`Sat of t | `Unsat] =
      ...
      let all_possible_lits  = SymbSet.to_list unassigned_set |> CCList.flat_map (fun s -> ([T.L.Neg s; T.L.Pos s])) in
      ...
      let init_st = {watch_lists=init_watc_list; occ_clause=occ_map;formula=clauses; stack=stk; model=cm; unassigned=unassigned_set;theory_state= T.initialize all_possible_lits} in
          solve init_st None
